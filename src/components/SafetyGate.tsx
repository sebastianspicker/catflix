...
interface SafetyGateProps {
  sceneTitle: string;
  onCancel: () => void;
  onContinue: (mode: PlaybackMode, setup: SetupContext) => void;
}
...
export function SafetyGate({ sceneTitle, onCancel, onContinue }: SafetyGateProps) {
  ...
  const [confirmed, setConfirmed] = useState<Record<(typeof checks)[number][0], boolean>>({ stableDevice: false, protectedCables: false, openExit: false, supervised: false });
  ...
  const begin = () => onContinue(mode, { stableDevice: true, protectedCables: true, openExit: true, supervised: true, roomLightBand: light, viewingDistanceBand: distance, ...(cat === 'not-recording' ? {} : { observedCat: cat }) });
  ...
  return <div className="modal-backdrop" role="presentation"><section ref={dialogRef} className="safety-gate encounter-setup" role="dialog" aria-modal="true" aria-labelledby="safety-title" tabIndex={-1}>
    ...
    <fieldset className="setup-checks"><legend>2 / Confirm before starting muted</legend>{checks.map(([key, label]) => <label key={key}><input type="checkbox" checked={confirmed[key]} onChange={(event) => setConfirmed((current) => ({ ...current, [key]: event.target.checked }))} /><span>{label}</span></label>)}</fieldset>
