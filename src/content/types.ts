...
function validateManifestFields(manifest: PartialManifest, errors: string[]) {
...
function validateManifestPresentation(manifest: PartialManifest, errors: string[]) {
...
function validateManifestEditorialMetadata(manifest: PartialManifest, errors: string[]) {
...
function validateManifestLimits(manifest: PartialManifest, errors: string[]) {
...
function validateManifestAssets(manifest: PartialManifest, errors: string[]) {
...
const isUniqueAssetForRevision = (value: unknown, revision: unknown, assetIds: AssetIdentifierSet, checksums: AssetIdentifierSet): value is AssetProvenance => {
...
function validateAssetCoverage(assets: readonly AssetProvenance[], manifest: PartialManifest, errors: string[]) {
...
function validateManifestAudio(manifest: PartialManifest, errors: string[]) {
...
const isMotionMetadata = (value: unknown): value is MotionMetadata => {
...
const hasValidMotionPace = (motion: PartialMotion): boolean => {
...
const hasNonEmptyValues = (value: unknown, allowed: readonly string[]): boolean => {
  return Array.isArray(value) && value.length > 0 && value.every((item) => isOneOf(item, allowed));
};

const hasValidOcclusion = (value: unknown): boolean => {
...
const isApparentSizeMetadata = (value: unknown): value is ApparentSizeMetadata => {
...
const isVisualPackage = (value: unknown): value is SceneVisualPackage => {
...
const isAudioProfile = (audio: PartialAudioProfile): audio is AudioProfile => {
...
const hasNonEmptyTextList = (value: unknown): value is readonly string[] => {
  return Array.isArray(value) && value.length > 0 && value.every(isNonEmptyText);
};

const hasValidAudioProvenance = (provenance: AudioProfile["provenance"], eventKinds: readonly string[]): boolean => {
...
const isEncounterMetadata = (value: unknown): value is EncounterEditorialMetadata => {
...
const hasEncounterText = (item: PartialEncounter): boolean => {
...
const hasValidPresentation = (value: unknown): boolean => {
...
const hasRiskRationale = (value: unknown): boolean => {
...
const hasEditorialClaims = (value: unknown): boolean => {
