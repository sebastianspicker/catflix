...
interface RefereeNotesProps { sceneTitle: string; observedCat?: 'Arri' | 'Ozzy' | 'Mika'; touchTimestamps: number[]; completed: boolean; onClose: () => void; onSave: (_draft: ObservationDraft) => void; }
...
export function RefereeNotes({ sceneTitle, observedCat, touchTimestamps, completed, onClose, onSave }: RefereeNotesProps) {
  ...
  return <div className="modal-backdrop" role="presentation"><section ref={dialogRef} className="notes-dialog observation-dialog" role="dialog" aria-modal="true" aria-labelledby="notes-title" tabIndex={-1}>
    ...
    <fieldset className="behavior-list"><legend>Observed vocabulary</legend>{behaviors.map((behavior) => <label key={behavior}><input type="checkbox" checked={selected.includes(behavior)} onChange={() => { toggle(behavior); }} /><span>{behavior}</span></label>)}</fieldset><label className="raw-note">Raw note<textarea value={rawNote} onChange={(event) => { setRawNote(event.target.value); }} placeholder="What did you observe? Preserve non-response and disengagement." /></label><p className="touch-summary">Accepted target contacts: {touchTimestamps.length}. Background taps are not recorded.</p><label className="acknowledgement"><input type="checkbox" checked={confirmed} onChange={(event) => { setConfirmed(event.target.checked); }} /><span>I confirm this descriptive local record.</span></label><div className="modal-actions"><button className="text-button" type="button" onClick={onClose}>Skip</button><button className="primary-button" type="button" disabled={!confirmed} onClick={() => { onSave({ endReason, vocabulary: selected, ...(safetyEvent ? { safetyEvent } : {}), physicalPlayHandoff, rawNote }); }}>Save observation</button></div>
  </section></div>;
}
