...
import { createPhaserSimulationRenderer } from "./simulationPhaserRenderer";
...
export function createSimulationHostRuntime(options: PhaserSimulationHostOptions): PhaserSimulationHost {
  ...
  const phaserRenderer = createPhaserSimulationRenderer({
  ...
  const update = (_time: number, delta: number): void => { if (running && !paused) phaserRenderer.render(tick(delta)); };
  ...
  const mountRenderer = (): void => {
    ...
    try {
      game = new Phaser.Game({ type: Phaser.AUTO, parent: options.container, transparent: true, scale: { mode: Phaser.Scale.RESIZE, width: "100%", height: "100%" }, scene: { preload: phaserRenderer.preload, create: phaserRenderer.create, update } });
    } catch {
  ...
  const start = (): void => {
    ...
    mountRenderer();
    if (!game) frameId = scheduleFrame(fallbackFrame);
  };
