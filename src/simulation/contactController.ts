...
import type { Point, SceneEvent, SceneScore, SimulationPreferences, TouchResponse } from "./types";
import { contactResponseFor, normalize } from "./simulationPrimitives";
...
export class ContactController {
  ...
  touch(sceneId: SceneId, score: SceneScore, preferences: SimulationPreferences, actors: MutableActor[], elapsedMs: number, point: Point, timestampMs: number, nextRandom: () => number): { result: TouchResponse; events: SceneEvent[] } {
    ...
    const actor = this.closestActor(actors, point);
    if (!actor || this.distance(actor, point) > score.subjectHitRadius) return { result: { accepted: false, refractoryUntilMs: this.state.refractoryUntilMs }, events: [] };
    const response = contactResponseFor(sceneId, actor.animationState, phaseAt(score, elapsedMs), score.touchPolicy.allowedResponses);
    this.state.refractoryUntilMs = timestampMs + score.touchPolicy.refractoryMs;
    actor.responseUntilMs = timestampMs + 650;
    this.applyResponse(actor, response, point, timestampMs);
  ...
  private applyResponse(actor: MutableActor, response: TouchResponse["response"], point: Point, timestampMs: number): void {
    if (response === "reroute" || response === "redirect") { const direction = normalize(actor.x - point.x, actor.y - point.y); actor.vx = direction.x; actor.vy = direction.y; return; }
  ...
  private recordRest(score: SceneScore, actors: MutableActor[], timestampMs: number, nextRandom: () => number, events: SceneEvent[]): void {
    this.acceptedTimes = [...this.acceptedTimes, timestampMs].filter((time) => timestampMs - time <= 20_000);
    if (this.acceptedTimes.length < 3 || this.state.reminder) return;
    const durationMs = 10_000 + Math.floor(nextRandom() * 2_001);
    ...
    this.state.reminder = { type: "contact-reminder", id: "three-contacts", acceptedContacts: 3, windowMs: 20_000, dismissible: true, editorialSafetyCap: true, atMs: timestampMs };
